...
void *ThreadCache::allocate(int memorySize) {
    ...
    int realSize = SizeClass::roundUp(memorySize);
    int index = SizeClass::getIndex(memorySize);
...
void *ThreadCache::fetchFromCentralCache(int index, int size) {
    ...
    if (totalCount == _freeLists[index].getCapacity()) _freeLists[index].getCapacity() += 20;
